...
import { ativarPushNotifications, getPushCapabilities } from '@/lib/pushClient'
...
export default function PushNotificationButton({ uid, compact = false, className = '' }) {
  ...
  async function handleAtivar() {
    ...
    try {
      ...
      setStatus(`Notificacoes ativas. Token salvo: ${tokenPreview(result.token)}`)
    } catch (error) {
  ...
  return (
    <div className={className}>
      ...
      {status ? (
        <div className={compact ? 'mt-2 text-[11px] font-bold text-cyan-200' : 'mt-2 rounded-2xl border border-cyan-300/15 bg-cyan-400/10 px-3 py-2 text-xs font-bold text-cyan-100'}>
          {status}
        </div>
      ) : null}
    </div>
